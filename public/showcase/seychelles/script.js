...
const contactModalPanel = contactModal?.querySelector('.contact-modal__panel');
...
function unlockPageScroll() {
  ...
  // Definitively force instant jump overriding CSS smooth-scroll
  window.scrollTo({ top: y, left: 0, behavior: 'instant' });
  
  // Extra safety frame to ensure it stays
  requestAnimationFrame(() => {
    window.scrollTo({ top: y, left: 0, behavior: 'instant' });
  });
}
...
function updateBackToTop() {
  ...
  const showBackToTop = pageTop > Math.max(window.innerHeight * 0.9, 720);

  backToTop?.classList.toggle('is-visible', showBackToTop && !hasOpenModal());
}
